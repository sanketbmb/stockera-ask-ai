...
serve(async (req: Request) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({
      ...
      exclusion_checks_for_hash: checkOrder.map(id => ({ check_id: id, enabled: jsonbBool(config.get(CHECK_CONFIG_MAP[id].enableKey)), threshold: config.get(CHECK_CONFIG_MAP[id].thresholdKey!)?.toString() ?? null }))
    }), { status: 200 });
  } catch (e) {
