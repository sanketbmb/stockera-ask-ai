...
// LIBRARY AUTH-VIEW SPLIT — authed reader that returns any AI report row
// regardless of is_public_library, so the sitewide "All AI Reports" feed
// (visible only to signed-in users) can open cards for reports the user
// does not own. Same safe projection as getPublicReportRow.
export const getReportRowForAuthedUser = createServerFn({ method: "POST" })
