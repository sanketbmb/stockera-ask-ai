...
function AnalysisPage() {
  ...
  const { data, isLoading, error, refetch, isFetching } = useQuery<OrchestratorResponse>({
    ...
    queryFn: async () => {
      ...
      // `candidates` array. Normalize via shared helper so /report and
      // /analysis render the same friendly picker.
      if (isSymbolAmbiguousError(data)) {
        return synthesizeAmbiguousPayload(data, symbol) as unknown as StockAnalysisPayload;
      }
